...
class listFunctions extends blogList {
	...
	function getListItems($justNumOfItems = false, $justItemArray = false){
		...
		if($this->globalPiVars['sword']){
			...
			// asdf add search over tt_content
		...
		if($this->localPiVars['groupBy']){	//set the group by value
			$groupBy = $this->localPiVars['groupBy'];
		}
		if($this->localPiVars['orderBy']){	//set order by
			$orderBy = $this->localPiVars['orderBy'];
		}
		if($this->localPiVars['orderByDir'] 	//set order by dir
			&& (strtolower($this->localPiVars['orderByDir'])=='asc'
			|| strtolower($this->localPiVars['orderByDir'])=='desc')){
			$orderBy .= ' '. strtoupper($this->localPiVars['orderByDir']);
		}
		...
		//add limit
		$limit = ($justNumOfItems == false) ? (intval($_GET['tx_t3blog_post_pointer']) ? intval($_GET['tx_t3blog_post_pointer']) : 0). ','. ($this->conf['numberOfRecords'] ? $this->conf['numberOfRecords'] : '10') : '';
