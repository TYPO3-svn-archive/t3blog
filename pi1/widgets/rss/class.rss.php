...
class rss extends tslib_pibase {
	...
	function getDate($value)	{
		...
		$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('date','tx_t3blog_post', 'deleted = 0 and hidden = 0 and uid = '.$value);
		while ($row =  $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
			$data = $row['date'];
		}
	...
	function getFkPost($value) {
		...
		$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('title','tx_t3blog_post', 'deleted = 0 and hidden = 0 and uid ='.$value.'','');
		while ($row =  $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
			$data = $row['title'];
		}
		return $data;
	...
	function getFkPostID($value) {
		...
		$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('fk_post','tx_t3blog_com', 'deleted = 0 and hidden = 0 and uid ='.$value.'','');
		while ($row =  $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
			$data = $row['fk_post'];
		}
		return $data;
	...
	function fieldWrap($field,$value)	{
		...
		switch($field) {
			...
			case 'uid':
                ...
                return '<link>'.htmlspecialchars((stripos('http://',t3lib_div::getIndpEnv('HTTP_HOST'))?'':'http://').t3lib_div::getIndpEnv('HTTP_HOST').'/index.php?id='.t3blog_div::getBlogPid().'&no_cache=1&tx_t3blog_pi1[blogList][showUid]='.$this->conf['feedItemLinkPrefix'].$postid).'</link>
	<guid>'.htmlspecialchars((stripos('http://',t3lib_div::getIndpEnv('HTTP_HOST'))?'':'http://').t3lib_div::getIndpEnv('HTTP_HOST').'/index.php?id='.t3blog_div::getBlogPid().'&no_cache=1&tx_t3blog_pi1[blogList][showUid]='.$this->conf['feedItemLinkPrefix'].$postid).'</guid>
