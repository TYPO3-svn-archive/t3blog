...
/**
 * Sorry, this code obviously has been taken from another class programmed
 * by somebody else.
 * But the previous author of this class here didn't mention where he
 * took it from.
 * 
 */
...
class tagCloud extends tslib_pibase {
	...
	function getTags_array(){
		...
		while ($resPost = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($posts)) {
			$tags = array_merge($tags, explode(',' , strtolower($resPost['tagClouds'])));
		}
		$tagArray = array();
		foreach ($tags as $tag) {
	...
	function calculateTagCloud() {
		...
		if( strtolower($this->sort) == 'tag' ){
			$_tags = $this->ArraySort2D( $_tags, 'tag', $this->descasc );
		} elseif( strtolower($this->sort) == 'count' ){
					usort( $_tags, array( $this, "CountCompare" ) );
		} else {
					shuffle( $_tags );
		}
		...
		if( in_array( strtolower($this->unit), array('em','ex','cm','in') ) ) {
			$this->maxsize *= 100;
			$this->minsize *= 100;
		}
		if( empty($this->thresholds) ) $this->thresholds = $this->maxsize - $this->minsize + 1;
		if( !in_array( strtolower($this->unit), array('em','ex','cm','in','pt','px','mm','pc','%') ) ) {
		...
		foreach( $_tags as $k=>$_v ) {
			...
			if( in_array( strtolower($this->unit), array('pt','px','mm','pc','%') ) ) :
			$s = "style=\"font-size:".$_s[$_v['count']].strtolower($this->unit).";".($_colors?"color:".$_colors[$_c[$_v['count']]-1].";":"")."\"";
			elseif( in_array( strtolower($this->unit), array('em','ex','cm','in' ) ) ) :
			$s = "style=\"font-size:".($s_[$_v['count']]/=100).strtolower($this->unit).";".($_colors?"color:".$_colors[$_c[$_v['count']]-1].";":"")."\"";
			else :
